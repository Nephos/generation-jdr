<?php
print_r($_POST);
$race = array('Ormazuriens','Chitiksss','Falaיs','Marflings','Etinיens','Synovingiens');
...
$output.='<h2>Crיation de personnage pour le Dernier Bastion</h2>';
if (!empty($_POST)) {
    switch ($_POST['race']) {
        case 0:
            if ($_POST['sexe']==0) {
    ...
    if ($_POST['sexe']==1 && $_POST['race']==3) {
        $output.='Pas de femelle chez les Marfling un male א donc יtי crיי. Les attributs sont les suivants :';
    } else {
    ...
    $output.='	<select name="race">
					...
					<option value="2">Falaיs</option>
					<option value="3">Marflings</option>
					<option value="4">Etinיens</option>
