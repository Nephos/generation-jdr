...
if (!empty($_POST)) {
    ...
    if (!empty($_POST['qtt'])&& $_POST['qtt']!=0 && $_POST['qtt']<1000) {
        for ($i=0;$i<$_POST['qtt'];$i++,$tabobjet++) {
            if (empty($_POST['Qualite']) || $_POST['Qualite']==0) {
                $_POST['Qualite'] = rand(1, 4);
            }
            if ($_POST['Qualite']==1) {
                ...
                $objet[$tabobjet] = getMagical($_POST['Qualite']);
            }
            compteur('objmag');
        }

        if ($_POST['qtt']>1) {
            $output.= '<h2>'.count($objet).' objets magiques</h2>';
        } else {
            $output.= '<h2>'.count($objet).' objet magique</h2>';
        }
