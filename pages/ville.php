...
if (!empty($_POST) || isset($_GET['nom'])) {
    ...
    if ($_POST['type']==0) {
        $typevillenum=rand(0, 2);
    } else {
        $typevillenum=$_POST['type']-1;
    }
    if ($_POST['taille']==0) {
        $taille=habitant(rand(1, 100));
    } else {
        $taille=$_POST['taille'];
    }
    ...
    if (array_sum($nbrhabitantparrace)<$nbrhabitant) {
    ...
    if ($nbrbat==0) {
        ...
        $output.='Il existe un seul commerce remarquable (<a href="#bouton_texte3">voir le commerce</a>). <br>';
    } else {
        $output.='Il existe dans '.$nomduvillage.', '.$nbrbat.' commerces remarquables (<a href="#bouton_texte3">voir la liste des commerces</a>). <br>';
    }
    //guildes
    if ($nbrguilde>0) {
        if ($nbrguilde==1) {
